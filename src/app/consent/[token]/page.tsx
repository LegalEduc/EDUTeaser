    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center p-6">
        <div className="text-center">
          <p className="text-red-600 text-[1.15rem] mb-4">{error}</p>
          <p className="text-slate text-[1rem]">문의: contact@legalcrew.co.kr</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-cream text-ink">
      <div className="max-w-[640px] mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="text-center mb-8 sm:mb-10">
          <p className="font-logo text-[1.15rem] mb-6 text-ink">
            LegalCrew <span className="italic">Academy</span>
          </p>
          <p className="text-caption font-medium text-slate mb-4">동의서</p>
          <h1 className="font-heading text-[22px] sm:text-[26px] font-bold mb-3">강의 동의서</h1>
          <p className="text-[1.05rem] text-slate">리걸크루 변호사 실전 압축 부트캠프</p>
        </div>

        {data.alreadySigned ? (
          <div className="text-center py-16">
            <div className="text-5xl mb-6 text-ink font-light">&#10003;</div>
            <h2 className="font-heading text-[24px] font-bold mb-4">이미 서명이 완료되었습니다</h2>
            <p className="text-[1.05rem] text-slate mb-8">
          ...
          <ConsentForm instructor={data.instructor} setting={data.setting} token={token} />
        )}

        <div className="mt-16 pt-8 border-t border-[#e2e2e2] text-center">
          <p className="text-[1rem] text-slate">
            문의: contact@legalcrew.co.kr &middot; 010-9131-4827
          </p>
          <p className="text-[1rem] text-slate-light mt-2">
